...
const ManageOwnManufacturing = () => {
  ...
  // State for all own manufacturing items
  const [manufacturingItems, setManufacturingItems] = useState([]);
  
  // Form state for selected own manufacturing item
  ...
  // Fetch all own manufacturing items on component mount
  useEffect(() => {
    fetchAllManufacturingItems();
  }, []);
  ...
  // Fetch all own manufacturing items from API
  const fetchAllManufacturingItems = async () => {
    setIsLoading(true);
    try {
      ...
      if (!response.ok) {
        throw new Error("Failed to fetch own manufacturing items data");
      }

      const result = await response.json();
      console.log("GET All Own Manufacturing Items API Response:", result);

      if (result.success && result.data && result.data.length > 0) {
        setManufacturingItems(result.data);
      } else {
        throw new Error("No own manufacturing items found");
      }
    } catch (error) {
      console.error("Error fetching own manufacturing items data:", error);
  ...
  // Fetch specific own manufacturing item data by ID
  const fetchManufacturingItemData = async (itemId) => {
    setIsLoading(true);
    try {
      console.log("Fetching own manufacturing item with ID:", itemId);
      const response = await fetch(
        `https://mahadevaaya.com/trilokayurveda/trilokabackend/api/our-focus-own-manufacturing-items/?id=${itemId}`,
        {
          method: "GET",
        }
      );
      ...
      if (!response.ok) {
        throw new Error(`Failed to fetch own manufacturing item data. Status: ${response.status}`);
      }

      const result = await response.json();
      console.log("GET Own Manufacturing Item API Response:", result);
      ...
      if (result.success) {
        ...
        if (Array.isArray(result.data)) {
          // If it's an array, find the own manufacturing item with matching ID
          itemData = result.data.find(item => item.id.toString() === itemId.toString());
          if (!itemData) {
            throw new Error(`Own manufacturing item with ID ${itemId} not found in response array`);
          }
          ...
          if (result.data.id.toString() === itemId.toString()) {
            ...
            throw new Error(`Returned own manufacturing item ID ${result.data.id} does not match requested ID ${itemId}`);
          }
        } else {
          throw new Error("Invalid own manufacturing item data structure in response");
        }
        ...
        throw new Error(result.message || "No own manufacturing item data found in response");
      }
    } catch (error) {
      console.error("Error fetching own manufacturing item data:", error);
      setMessage(error.message || "An error occurred while fetching own manufacturing item data");
      setVariant("danger");
      setShowAlert(true);
    } finally {
      setIsLoading(false);
  ...
  // Handle own manufacturing item card click
  const handleItemClick = (itemId) => {
    console.log("Own manufacturing item card clicked with ID:", itemId);
    fetchManufacturingItemData(itemId);
  };
  ...
  const resetForm = () => {
    if (selectedItemId) {
      fetchManufacturingItemData(selectedItemId);
    }
    setIconPreview(null);
    setImagePreview(null);
    setIsEditing(false);
  ...
  // Go back to own manufacturing item list
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log("Submitting data for own manufacturing item ID:", formData.id);
      ...
      if (formData.icon || formData.image) {
        ...
        const url = `https://mahadevaaya.com/trilokayurveda/trilokabackend/api/our-focus-own-manufacturing-items/?id=${formData.id}`;
        ...
        if (!response.ok) {
          ...
          throw new Error(
            (errorData && errorData.message) ||
              "Failed to update own manufacturing item"
          );
        }
        ...
        if (result.success) {
          setMessage("Own manufacturing item updated successfully!");
          setVariant("success");
          setShowAlert(true);
          setIsEditing(false);
          ...
          // Update the own manufacturing item in the list
          if (result.data) {
            let updatedItem;
            if (Array.isArray(result.data)) {
            ...
            if (updatedItem) {
              setManufacturingItems(prevItems => 
                prevItems.map(item => 
          ...
          throw new Error(
            result.message || "Failed to update own manufacturing item"
          );
        }
      } else {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            errorData.message || "Failed to update own manufacturing item"
          );
        }
      ...
      console.error("Error updating own manufacturing item:", error);
  ...
  const handleDelete = async () => {
    if (!window.confirm("Are you sure you want to delete this own manufacturing item?")) {
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          (errorData && errorData.message) ||
            "Failed to delete own manufacturing item"
        );
      }
      ...
      if (result.success) {
        setMessage("Own manufacturing item deleted successfully!");
        setVariant("success");
        setShowAlert(true);
        
        // Remove the own manufacturing item from the list
        setManufacturingItems(prevItems => 
        ...
        throw new Error(
          result.message || "Failed to delete own manufacturing item"
        );
      }
    } catch (error) {
      console.error("Error deleting own manufacturing item:", error);
      let errorMessage = "An unexpected error occurred. Please try again.";
      ...
      <div className="dashboard-container">
        ...
        <div className="main-content">
          ...
          <Container fluid className="dashboard-body dashboard-main-container">
            <h1 className="page-title">Manage Own Manufacturing</h1>
            ...
            {isLoading ? (
              <div className="text-center py-5">
                ...
                <p className="mt-2">Loading Own Manufacturing Items...</p>
              </div>
            ) : (
              <>
                {!selectedItemId ? (
                  // Own Manufacturing Items List View
                  <>
                    <Row className="mb-4">
                      <Col>
                        <h2 className="mb-4">Select an Own Manufacturing Item to Edit</h2>
                        {manufacturingItems.length === 0 ? (
                          <Alert variant="info">
                            No own manufacturing items found. Please create own manufacturing items first.
                          </Alert>
                        ) : (
                          <Row>
                            {manufacturingItems.map((item) => (
                              <Col md={6} lg={4} className="mb-4" key={item.id}>
                                <Card 
                                  className="h-100 manufacturing-card profile-card" 
                  ...
                  // Own Manufacturing Item Edit View
                  <>
                    <div className="d-flex justify-content-between align-items-center mb-4">
                      <Button variant="outline-secondary" onClick={backToItemList}>
                        <FaArrowLeft /> Back to Own Manufacturing Items
                      </Button>
                      {!isEditing && (
