...
const ManageAim = () => {
  ...
  // State for all aim items
  const [aimItems, setAimItems] = useState([]);
  
  // Form state for selected aim item
  ...
  // Fetch all aim items on component mount
  useEffect(() => {
    fetchAllAimItems();
  }, []);
  ...
  // Fetch all aim items from API
  const fetchAllAimItems = async () => {
    setIsLoading(true);
    try {
      const response = await fetch(
        "https://mahadevaaya.com/trilokayurveda/trilokabackend/api/ouraim-item/",
        {
          method: "GET",
        }
      );
      ...
      if (!response.ok) {
        throw new Error("Failed to fetch aim items data");
      }

      const result = await response.json();
      console.log("GET All Aim Items API Response:", result);

      if (result.success && result.data && result.data.length > 0) {
        setAimItems(result.data);
      } else {
        throw new Error("No aim items found");
      }
    } catch (error) {
      console.error("Error fetching aim items data:", error);
  ...
  // Fetch specific aim item data by ID
  const fetchAimItemData = async (itemId) => {
    setIsLoading(true);
    try {
      console.log("Fetching aim item with ID:", itemId);
      const response = await fetch(
        `https://mahadevaaya.com/trilokayurveda/trilokabackend/api/ouraim-item/?id=${itemId}`,
        {
          method: "GET",
        }
      );
      ...
      if (!response.ok) {
        throw new Error(`Failed to fetch aim item data. Status: ${response.status}`);
      }

      const result = await response.json();
      console.log("GET Aim Item API Response:", result);
      ...
      if (result.success) {
        ...
        if (Array.isArray(result.data)) {
          // If it's an array, find the aim item with matching ID
          itemData = result.data.find(item => item.id.toString() === itemId.toString());
          if (!itemData) {
            throw new Error(`Aim item with ID ${itemId} not found in response array`);
          }
          ...
          if (result.data.id.toString() === itemId.toString()) {
            ...
            throw new Error(`Returned aim item ID ${result.data.id} does not match requested ID ${itemId}`);
          }
        } else {
          throw new Error("Invalid aim item data structure in response");
        }
        ...
        throw new Error(result.message || "No aim item data found in response");
      }
    } catch (error) {
      console.error("Error fetching aim item data:", error);
      setMessage(error.message || "An error occurred while fetching aim item data");
      setVariant("danger");
      setShowAlert(true);
    } finally {
      setIsLoading(false);
  ...
  // Handle aim item card click
  const handleItemClick = (itemId) => {
    console.log("Aim item card clicked with ID:", itemId);
    fetchAimItemData(itemId);
  };
  ...
  const resetForm = () => {
    if (selectedItemId) {
      fetchAimItemData(selectedItemId);
    }
    setImagePreview(null);
    setIsEditing(false);
    setShowAlert(false);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log("Submitting data for aim item ID:", formData.id);
      ...
      if (formData.image) {
        ...
        const url = `https://mahadevaaya.com/trilokayurveda/trilokabackend/api/ouraim-item/?id=${formData.id}`;
        ...
        if (result.success) {
          setMessage("Aim item updated successfully!");
          setVariant("success");
          setShowAlert(true);
          setIsEditing(false);
          ...
          // Update the aim item in the list
          if (result.data) {
            let updatedItem;
            if (Array.isArray(result.data)) {
          ...
          throw new Error(
            result.message || "Failed to update aim item"
          );
        }
      } else {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            errorData.message || "Failed to update aim item"
          );
        }
      ...
      console.error("Error updating aim item:", error);
      ...
      <div className="dashboard-container">
        ...
        <div className="main-content">
          ...
          <Container fluid className="dashboard-body dashboard-main-container">
            <h1 className="page-title">Manage Aim</h1>
            ...
            {isLoading ? (
              <div className="text-center py-5">
                ...
                <p className="mt-2">Loading Aim Items...</p>
              </div>
            ) : (
              <>
                {!selectedItemId ? (
                  ...
                  <>
                    <Row className="mb-4">
                      <Col>
                        <h2 className="mb-4">Select an Aim Item to Edit</h2>
                        {aimItems.length === 0 ? (
                          <Alert variant="info">
                            No aim items found. Please create aim items first.
                          </Alert>
                        ) : (
                          <Row>
                            {aimItems.map((item) => (
                              <Col md={6} lg={4} className="mb-4" key={item.id}>
                                <Card 
                                  className="h-100 aim-card profile-card" 
