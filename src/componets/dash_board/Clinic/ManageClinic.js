...
const ManageClinic = () => {
  ...
  // State for all clinic items
  const [clinicItems, setClinicItems] = useState([]);
  
  // Form state for selected clinic item
  ...
  // Fetch all clinic items on component mount
  useEffect(() => {
    fetchAllClinicItems();
  }, []);
  ...
  // Fetch all clinic items from API
  const fetchAllClinicItems = async () => {
    setIsLoading(true);
    try {
      const response = await fetch(
        "https://mahadevaaya.com/trilokayurveda/trilokabackend/api/our-clinic-item/",
        {
          method: "GET",
        }
      );
      ...
      if (!response.ok) {
        throw new Error("Failed to fetch clinic items data");
      }

      const result = await response.json();
      console.log("GET All Clinic Items API Response:", result);

      if (result.success && result.data && result.data.length > 0) {
        setClinicItems(result.data);
      } else {
        throw new Error("No clinic items found");
      }
    } catch (error) {
      console.error("Error fetching clinic items data:", error);
  ...
  // Fetch specific clinic item data by ID
  const fetchClinicItemData = async (itemId) => {
    setIsLoading(true);
    try {
      console.log("Fetching clinic item with ID:", itemId);
      const response = await fetch(
        `https://mahadevaaya.com/trilokayurveda/trilokabackend/api/our-clinic-item/?id=${itemId}`,
        {
          method: "GET",
        }
      );
      ...
      if (!response.ok) {
        throw new Error(`Failed to fetch clinic item data. Status: ${response.status}`);
      }

      const result = await response.json();
      console.log("GET Clinic Item API Response:", result);
      ...
      if (result.success) {
        ...
        if (Array.isArray(result.data)) {
          // If it's an array, find the clinic item with matching ID
          itemData = result.data.find(item => item.id.toString() === itemId.toString());
          if (!itemData) {
            throw new Error(`Clinic item with ID ${itemId} not found in response array`);
          }
          ...
          if (result.data.id.toString() === itemId.toString()) {
            ...
            throw new Error(`Returned clinic item ID ${result.data.id} does not match requested ID ${itemId}`);
          }
        } else {
          throw new Error("Invalid clinic item data structure in response");
        }
        ...
        throw new Error(result.message || "No clinic item data found in response");
      }
    } catch (error) {
      console.error("Error fetching clinic item data:", error);
      setMessage(error.message || "An error occurred while fetching clinic item data");
      setVariant("danger");
      setShowAlert(true);
    } finally {
      setIsLoading(false);
  ...
  // Handle clinic item card click
  const handleItemClick = (itemId) => {
    console.log("Clinic item card clicked with ID:", itemId);
    fetchClinicItemData(itemId);
  };
  ...
  const resetForm = () => {
    if (selectedItemId) {
      fetchClinicItemData(selectedItemId);
    }
    setImagePreview(null);
    setIsEditing(false);
    setShowAlert(false);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log("Submitting data for clinic item ID:", formData.id);
      ...
      if (formData.image) {
        ...
        const url = `https://mahadevaaya.com/trilokayurveda/trilokabackend/api/our-clinic-item/?id=${formData.id}`;
        ...
        if (result.success) {
          setMessage("Clinic item updated successfully!");
          setVariant("success");
          setShowAlert(true);
          setIsEditing(false);
          ...
          // Update the clinic item in the list
          if (result.data) {
            let updatedItem;
            if (Array.isArray(result.data)) {
          ...
          throw new Error(
            result.message || "Failed to update clinic item"
          );
        }
      } else {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            errorData.message || "Failed to update clinic item"
          );
        }
      ...
      console.error("Error updating clinic item:", error);
          ...
          <Container fluid className="dashboard-body dashboard-main-container">
            ...
            {isLoading ? (
              <div className="text-center py-5">
                ...
                <p className="mt-2">Loading Clinic Items...</p>
              </div>
            ) : (
              <>
                {!selectedItemId ? (
                  // Clinic Items List View
                  <>
                    <Row className="mb-4">
                      <Col>
                        <h2 className="mb-4">Select a Clinic Item to Edit</h2>
                        {clinicItems.length === 0 ? (
                          <Alert variant="info">
                            No clinic items found. Please create clinic items first.
                          </Alert>
                        ) : (
                          <Row>
                            {clinicItems.map((item) => (
                              <Col md={6} lg={4} className="mb-4" key={item.id}>
                                <Card 
                                  className="h-100 clinic-card profile-card" 
