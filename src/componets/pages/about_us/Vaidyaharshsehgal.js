...
function Vaidyaharshsehgal() {
    ...
    const fetchProfileData = async () => {
      try {
        ...
        if (response.data.success && response.data.data.length > 0) {
          ...
          const harshProfile =
            response.data.data.find((profile) => profile.id === 11) ||
  ...
  if (awardsModule && awardsModule.description) {
    ...
    const awardLines = awardsModule.description
      .split("\n")
    ...
    awards = awardLines.map((line, index) => {
      ...
      const title = year
        ? line.substring(0, line.indexOf(year)).trim()
        : line.trim();
