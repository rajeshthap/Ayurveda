...
function Care() {
  ...
  useEffect(() => {
    const fetchCareData = async () => {
      setIsLoading(true);
      try {
        const response = await fetch(`${API_BASE}/api/who-care-item/`, {
          method: 'GET',
        });
  ...
  const renderModules = () => {
    if (!careData || !careData.module || careData.module.length === 0) {
      return null;
    }

    return (
      <div className="modules-container mt-4">
        {careData.module.map((module, index) => {
          // For the Care API, module is an array with [title, description]
    ...
    <div className="ayur-bgcover ayur-about-sec">
      <div className='about-bg'>
        <div className='ayur-bread-content'>
          <h2 style={{ fontWeight: 'bold' }}>Care</h2>
          <div className="ayur-bread-list">
            <span>
                <Link to="/" >Home</Link>
            </span>
            <span className="ayur-active-page">/ Care</span>
          </div>
        </div>
      </div>
                ...
                <div className="ayur-heading-wrap ayur-about-head">
                  {isLoading ? (
                    <div className="text-center py-5">
                      ...
                      <p className="mt-2">Loading Care content...</p>
                    </div>
                  ) : error ? (
                      ...
                      <Row className="mt-4">
                        ...
                        <Col lg={8} md={12} sm={12} className="d-flex flex-column">
                          <div className="about-content text-start">
                            <h4 style={{ fontWeight: 'bold', fontSize: '2rem' }}>{careData.title}</h4>
                            <div 
                              className="about-description"
                              style={{ fontSize: '1.2rem' }}
                              dangerouslySetInnerHTML={{ 
                                __html: careData.description.replace(/\r\n\r\n/g, '<br /><br />').replace(/\r\n/g, '<br />') 
                              }}
                            />
