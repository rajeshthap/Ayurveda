...
function Call() {
  ...
  useEffect(() => {
    const fetchCallData = async () => {
      setIsLoading(true);
      try {
        const response = await fetch(`${API_BASE}/api/call-to-action-item/`, {
          method: 'GET',
        });
  ...
  const renderModules = () => {
    if (!callData || !callData.module || callData.module.length === 0) {
      return null;
    }

    return (
      <div className="modules-container mt-4">
        {callData.module.map((module, index) => {
          // For the Call API, module is an array with [title, description]
      ...
      <div className='about-bg'>
        <div className='ayur-bread-content'>
          ...
          <div className="ayur-bread-list">
            ...
            <span className="ayur-active-page">/ Call</span>
          </div>
        </div>
      </div>
                ...
                <div className="ayur-heading-wrap ayur-about-head">
                  {isLoading ? (
                    <div className="text-center py-5">
                      ...
                      <p className="mt-2">Loading Call content...</p>
                    </div>
                  ) : error ? (
