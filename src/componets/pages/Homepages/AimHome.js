...
const AimHome = () => {
  ...
  useEffect(() => {
    const fetchAimData = async () => {
      setIsLoading(true)
      try {
        const response = await fetch(`${API_BASE}/api/ouraim-item/`, { method: 'GET' })
        if (!response.ok) throw new Error('Failed to fetch Aim data')
        ...
        <div className="row">
          <div className="col-lg-12 col-md-12 col-sm-12">
            <div className="ayur-heading-wrap">
              <h3>{aimData ? aimData.title : 'What Ayurveda Aims To Do'}</h3>
              <h5>{aimData ? getDescriptionSubtitle(aimData.description) : 'Loading...'}</h5>
            </div>
          </div>
        </div>
          ...
          <div className="col-lg-8 col-md-12 col-sm-12">
            {isLoading ? (
              ...
              <>
                ...
                {aimData && aimData.module && aimData.module.length > 0 && (
                  <div className="ayur-about-head">
                    {aimData.module.slice(0, 2).map((module, idx) => {
                    ...
                    {aimData.module.length > 2 && <p>...</p>}
                  </div>
                )}

                <Link to="/Aim" className="ayur-btn" onClick={scrollToTop}>
                  Know More
                </Link>
              </>
            )}
